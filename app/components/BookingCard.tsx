...
export default function BookingCard(props: BookingCardProps) {
    ...
    return (
        <div className="flex flex-col md:flex-row border border-[#e2dcd0] rounded-2xl overflow-hidden bg-white text-zinc-800 w-full mb-6 shadow-sm hover:shadow-md transition-all duration-300 text-left relative z-10">
            ...
            <div className="flex-1 p-6 flex flex-col justify-between">
                <div>
                    <div className="flex justify-between items-start mb-2">
                        <h2 className="text-xl font-bold font-poppins text-text">
                            {calculatedNights} nights in {item.cabins?.name}
                        </h2>

                        <span className="bg-green-50 text-green-700 border border-green-200 px-3 py-1 text-xs font-bold rounded-full uppercase tracking-wide">
                            {item.status}
                        </span>

                    </div>
                    <p className="text-zinc-500 font-mulish text-sm">
                ...
                <div className="flex justify-between items-end mt-6">
                    <div className="flex items-center gap-3">
                        <span className="text-text font-bold text-2xl font-poppins">
                            ${item.totalPrice}
                        </span>
                        <span className="text-zinc-500 flex items-center text-sm font-mulish">
                            <span className="mr-2 text-text">&bull;</span> {item.numGuests} guest{item.numGuests > 1 ? 's' : ''}
                        </span>
                    </div>
                    <p className="text-xs text-zinc-400 font-mulish">
                        Booked {dayjs(item.createdAt).format('DD/MM/YYYY, h:mm A')}
                    </p>
                </div>
            </div>

            <div className="w-full md:w-32 shrink-0 flex flex-row md:flex-col border-t md:border-t-0 md:border-l border-[#e2dcd0]">

                <Link href={`/account/bookings/edit/${item.id}`} className="flex-1 flex items-center justify-center gap-2 text-sm font-semibold text-text hover:bg-[#faf8f5] transition-colors border-r md:border-r-0 md:border-b border-[#e2dcd0] group tracking-wider font-poppins py-4 md:py-0">
                ...
                <button onClick={() => handleDelete(item.id)} className="flex-1 flex items-center justify-center gap-2 text-sm font-semibold text-text hover:bg-red-50 hover:text-red-600 transition-colors group tracking-wider font-poppins py-4 md:py-0">
